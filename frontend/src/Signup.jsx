      ...
      <div className="w-full max-w-6xl min-h-[650px] bg-white rounded-[28px] shadow-[0_25px_70px_rgba(30,41,59,0.12)] overflow-hidden grid lg:grid-cols-2">
        <div className="relative hidden lg:flex flex-col justify-between overflow-hidden bg-gradient-to-br from-indigo-700 via-indigo-600 to-violet-700 p-12 text-white">
          ...
          <div className="relative z-10 max-w-lg">
            <div className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-2 text-sm text-indigo-100 backdrop-blur-sm mb-7">
              ...
              Create your teaching workspace
            </div>
            <h2 className="text-5xl font-bold leading-[1.08] tracking-tight">
              Start your
              <br />
              <span className="text-cyan-200">teacher account.</span>
            </h2>
            <p className="mt-6 max-w-md text-base leading-7 text-indigo-100">
              Join EduAssist AI to plan lessons, create assessments, and manage classroom content in one place.
            </p>
            <div className="mt-9 space-y-4">
              <Feature text="Quick teacher onboarding" />
              <Feature text="Secure password storage" />
              <Feature text="Instant account creation" />
              <Feature text="Ready for login after signup" />
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-center p-6 sm:p-10 lg:p-12">
          <div className="w-full max-w-md">
            <div className="mb-8">
              <h2 className="text-3xl sm:text-4xl font-bold tracking-tight text-gray-900">Create teacher account</h2>
              <p className="mt-3 text-sm sm:text-base text-gray-500">Register to start using your teaching workspace.</p>
            </div>

            <form onSubmit={handleSignup}>
              <div className="mb-5">
                <label htmlFor="name" className="mb-2 block text-sm font-semibold text-gray-700">Name</label>
                <input id="name" type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Teacher name" required className="w-full rounded-xl border border-gray-200 bg-gray-50 py-3.5 px-4 text-sm text-gray-900 placeholder:text-gray-400 outline-none transition focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-500/10" />
              </div>

              <div className="mb-5">
                <label htmlFor="email" className="mb-2 block text-sm font-semibold text-gray-700">Email</label>
                <input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="teacher@example.com" required className="w-full rounded-xl border border-gray-200 bg-gray-50 py-3.5 px-4 text-sm text-gray-900 placeholder:text-gray-400 outline-none transition focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-500/10" />
              </div>

              <div className="mb-5">
                <label htmlFor="password" className="mb-2 block text-sm font-semibold text-gray-700">Password</label>
                <div className="relative">
                  <input id="password" type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Create a password" required className="w-full rounded-xl border border-gray-200 bg-gray-50 py-3.5 px-4 pr-12 text-sm text-gray-900 placeholder:text-gray-400 outline-none transition focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-500/10" />
              ...
              <div className="mb-4">
                <label htmlFor="confirmPassword" className="mb-2 block text-sm font-semibold text-gray-700">Confirm password</label>
                <input id="confirmPassword" type={showPassword ? "text" : "password"} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Repeat your password" required className="w-full rounded-xl border border-gray-200 bg-gray-50 py-3.5 px-4 text-sm text-gray-900 placeholder:text-gray-400 outline-none transition focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-500/10" />
              </div>
            ...
            <p className="mt-8 text-center text-sm text-gray-500">
              ...
              <button type="button" onClick={() => onNavigate('/')} className="ml-1 font-semibold text-indigo-600 hover:text-indigo-700 transition">Sign in</button>
            </p>
          </div>
        </div>
      </div>
